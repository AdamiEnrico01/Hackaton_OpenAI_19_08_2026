...
export function ImageStudio() {
  ...
  async function createImage() {
    ...
    try {
      ...
      const result = await response.json().catch(() => null) as { image?: string; error?: string };
      if (!response.ok || !result?.image) {
        throw new Error(result?.error ?? "Não foi possível gerar a imagem agora.");
      }
      setGeneratedImage(result.image);
      setState("ready");
    } catch (error) {
      setState("error");
      setNotice(error instanceof Error ? error.message : "Não foi possível gerar a imagem agora.");
    }
  }
